...
| Android SQLCipher release-candidate evidence | Phase 41 evidence exists, but the exact release-candidate build still needs fresh runtime proof. | Run the Android development-client verification path for the release-candidate build and update `docs/release/android-sqlcipher-evidence.md`. |
| iOS SQLCipher runtime evidence | Current workspace is Windows; iOS runtime evidence requires macOS/Xcode. | Run the Phase 42 macOS flow, capture simulator/device logs, and update `docs/release/ios-sqlcipher-evidence.md` for `Available - schema v1`. |
| Non-exportable device signing keys | Current SecureStore-backed fallback isolates key access but still documents native non-exportable key provider as required. | Implement and review native Keychain/Keystore signing providers, then update `deviceSigningKeyStore`. |
