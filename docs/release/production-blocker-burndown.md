...
| --- | --- | --- |
| Reviewed Signal/libsignal one-to-one adapter | Real production E2EE is not implemented. The app has a provider boundary, Phase 76 integration plan, Gate 2 feasibility spike, and fail-closed gate. | Implement the official libsignal Android/iOS native bridge behind `SignalNativeAdapter` and `SignalOneToOneCryptoAdapter`, add interoperability vectors, and obtain cryptography review. |
