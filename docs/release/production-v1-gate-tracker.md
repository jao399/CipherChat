...
| 2. Real reviewed Signal/libsignal adapter | Active workstream; still blocking | Adapter design review, dependency/license review, Android Java/Kotlin bridge proof, iOS Swift bridge proof, official libsignal version pin, X3DH/prekey vectors, Double Ratchet session tests, interoperability evidence, external crypto review sign-off | Research official libsignal Java/Swift path; design React Native native module boundary; build Android bridge to `org.signal` libsignal package; build iOS bridge to `LibSignalClient`; wire behind existing Signal adapter gate; keep production send fail-closed until evidence exists | Native unit tests; JS adapter contract tests; X3DH prekey validation; Double Ratchet send/receive vectors; changed-key/safety-number tests; encrypted session storage tests; live send fail-closed regression tests | Approve licensing risk and implementation scope; provide review budget; choose whether to proceed with AGPL-compatible open-source distribution or obtain legal guidance | Signal libsignal API stability/support policy, AGPLv3 license review, native Android/iOS build environment, external cryptography reviewer | Reviewed adapter is installed, production readiness checks pass, live one-to-one sends use reviewed Signal/libsignal provider, prototype provider remains blocked in production, and external review accepts the implementation |
