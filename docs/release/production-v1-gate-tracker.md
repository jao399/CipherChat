...
Create the Android Kotlin/Java bridge skeleton for `SignalNativeAdapter` using pinned official `org.signal` libsignal artifacts in an Expo development build, with no production readiness flag until X3DH, Double Ratchet, encrypted session storage, and review evidence pass.
