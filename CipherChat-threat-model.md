...
| TM-003 | Authenticated spammer | Attacker has one or more valid sessions. | Submit many fanout envelopes or large ciphertext/header payloads to grow DB/queue. | Storage exhaustion, queue latency, degraded delivery. | PostgreSQL, Redis, worker availability. | Route-aware rate limiting and envelope caps live in `apps/api/src/security/abusePolicy.ts`; fanout queues generic jobs in `apps/api/src/routes/messageRoutes.ts`. | Per-account quotas and queue-depth backpressure are still pending. | Add account/device quotas, queue-depth controls, and operational alerts around the current route and payload limits. | Metrics for envelope count, fanout size, queue depth, DB growth, and rate-limit triggers. | Medium | Medium | medium |
...
| TM-008 | External attacker or bot | API is internet reachable. | Abuse unauthenticated account creation/challenge endpoints for enumeration, cost, or account squatting. | Service abuse, account namespace exhaustion, operational noise. | Accounts, challenges, DB capacity. | Route-aware rate limiting covers account creation, device challenges, and device sessions in `apps/api/src/security/abusePolicy.ts`; challenge only issued for active existing devices in `apps/api/src/repositories/prismaSessionRepository.ts`. | Captcha/proof-of-work, invite policy, email/phone proof, and namespace reservation policy are still pending. | Add account creation proofing, username normalization/reservation, durable reputation signals, and suspicious creation detection. | Metrics for account creation velocity, failed challenges, IP/device reputation, and username-squatting patterns. | Medium | Medium | medium |
